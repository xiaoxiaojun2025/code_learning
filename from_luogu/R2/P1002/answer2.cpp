...
ll waysOfQuestion(Position destination,vector<Position> horses){
    vector<vector<ll>> dp(destination.x+1,vector<ll>(destination.y+1,0));
    vector<vector<bool>> blocked(destination.x+1,vector<bool> (destination.y+1,false));
    for(auto horse:horses){
        if(horse.x>=0&&horse.y>=0) blocked.at(horse.x).at(horse.y)=true;
    }
    for(int i=0;i<=destination.x;i++){
        for(int j=0;j<=destination.y;j++){
            if(blocked.at(i).at(j)) dp.at(i).at(j)=0;
            else{
                if(i==0&&j==0) dp[i][j]=1;
